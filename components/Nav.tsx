...
function NavTina(props: { data: HomeQuery; query: string; variables: HomeQueryVariables }) {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-4 px-4 pb-2 bg-gradient-to-b from-background/80 to-transparent pointer-events-none">
      <header className="glass rounded-full px-6 py-3 flex items-center justify-between w-full max-w-4xl pointer-events-auto transition-all shadow-lg shadow-black/5 dark:shadow-white/5">
        <Link
          href="/"
          className="text-lg font-heading font-bold text-primary dark:text-white tracking-tight"
          data-tina-field={tinaField(content.footer, 'name')}
        >
          {content.footer?.name}
        </Link>
        <nav className="hidden md:flex items-center gap-6">
...
export default function Nav(props: NavProps) {
  if (props.data && props.query && props.variables) {
    return <NavTina data={props.data} query={props.query} variables={props.variables} />
  }
